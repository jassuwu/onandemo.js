...
- **Companion** — the animated character that chases the cursor (the generalized cat). _Avoid in prose_: pet, mascot, follower (fine as npm keywords); sprite (the companion is driven _by_ sprites; it isn't one).
- **Preset** — a bundled, ready-to-run companion: a sheet plus its frame map (tuning rides inside the map), selectable by name. v1 ships `neko` (the homage) plus CC0 friends. _Avoid_: skin (the forks' word for re-arting the cat; a preset swaps the whole companion), theme.
